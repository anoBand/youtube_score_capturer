...
<br>
'평균 고정 시간'이라는 네이밍이 깔끔하진 않지만, 풀어 설명하자면 같은 이미지가 2초 이상 유지될 경우 진짜 악보로 인식해 pdf로 추출할 이미지로 저장하도록 했다. 하지만 동적으로 바뀔 다음 악보, 바뀐 다음 악보의 자투리 이미지가 같이 저장되는 문제는 찝찝하게 남아있었지만 이를 자동으로 잘라내는 기능을 추가하기에는 구현 난이도가 또 올라가기 때문에 유기했다.  
...
<br>
대부분 바이브코딩으로 만든 셈이라 컴공인 입장에서 아쉽긴 하지만 역으로 바이브코딩의 단점과 일종의 사용 가이드도 제작하면서 알게 되었고 이후로 코드리뷰도 제대로 해야겠다는 생각이 든다.
